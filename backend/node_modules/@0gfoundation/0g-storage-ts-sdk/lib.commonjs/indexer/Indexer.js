...
const decryption_js_1 = require("./decryption.js");
...
class Indexer extends open_jsonrpc_provider_1.HttpProvider {
    ...
    async selectNodes(expectedReplica, method = 'min') {
        ...
        const [trusted, ok] = (0, index_js_1.selectNodes)(nodes.trusted, expectedReplica, method);
        if (!ok) {
            return [
                [],
        ...
        const clients = trusted.map((node) => new index_js_3.StorageNode(node.url));
        return [clients, null];
    }
    // ─── Upload ───────────────────────────────────────────────────────────
    async newUploaderFromIndexerNodes(blockchain_rpc, signer, expectedReplica, opts) {
        ...
        const flow = (0, utils_js_1.getFlowContract)(status.networkIdentity.flowAddress, signer);
        console.log('Selected nodes:', clients);
        const uploader = new index_js_2.Uploader(clients, blockchain_rpc, flow, opts?.gasPrice, opts?.gasLimit);
    ...
    async upload(file, blockchain_rpc, signer, uploadOpts, retryOpts, opts) {
        ...
        const [uploader, err] = await this.newUploaderFromIndexerNodes(blockchain_rpc, signer, mergedOpts.expectedReplica, opts);
    ...
    async downloadFragments(rootHashes, filePath, proof) {
        ...
        const fs = await Promise.resolve().then(() => __importStar(require(/* webpackIgnore: true */ 'fs')));
        let outFile;
        try {
    ...
    async downloadToBlob(rootHashOrHashes, opts = {}) {
        if (Array.isArray(rootHashOrHashes)) {
    ...
    async downloadFragmentsToBlob(rootHashes, opts) {
    ...
    async downloadSingleToBlob(rootHash, opts) {
        ...
        const [rawBlob, dlErr] = await downloader.downloadToBlob(rootHash, opts.proof ?? false);
        if (dlErr !== null)
            return [new Blob(), dlErr];
        if (!opts.decryption)
            return [rawBlob, null];
        ...
        const symmetricKey = typeof opts.decryption.symmetricKey === 'string'
            ? hexStringToBytes(opts.decryption.symmetricKey)
            : opts.decryption.symmetricKey;
        const { bytes, decrypted } = (0, decryption_js_1.tryDecrypt)(encrypted, {
            symmetricKey,
    ...
    async peekHeader(rootHash) {
        ...
        const [rawBlob, dlErr] = await downloader.downloadToBlob(rootHash, false);
        if (dlErr !== null)
            return [null, dlErr];
        ...
        const prefix = new Uint8Array(await rawBlob.slice(0, prefixSize).arrayBuffer());
        try {
            return [(0, encryption_js_1.parseEncryptionHeader)(prefix), null];
        }
        catch {
            return [null, null];
        }
    ...
    async newDownloaderFromIndexerNodes(rootHash) {
        ...
        const [selected, ok] = (0, index_js_1.selectNodes)(locations, 1, 'random');
        if (!ok) {
            return [
                null,
        ...
        console.log(`Selected ${selected.length} of ${locations.length} nodes for ${rootHash}`);
        const clients = selected.map((node) => new index_js_3.StorageNode(node.url));
        return [new index_js_2.Downloader(clients), null];
    }
}
