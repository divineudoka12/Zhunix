...
import { parseEncryptionHeader, } from '../common/encryption.js';
...
export class Indexer extends HttpProvider {
    ...
    async selectNodes(expectedReplica, method = 'min') {
        ...
        const clients = trusted.map((node) => new StorageNode(node.url));
        return [clients, null];
    }
    // ─── Upload ───────────────────────────────────────────────────────────
    async newUploaderFromIndexerNodes(blockchain_rpc, signer, expectedReplica, opts) {
        ...
        const uploader = new Uploader(clients, blockchain_rpc, flow, opts?.gasPrice, opts?.gasLimit);
    ...
    async downloadFragmentsToBlob(rootHashes, opts) {
        ...
        const plaintexts = tryDecryptFragments(rawParts, symmetricKey, privateKey);
        if (plaintexts === null) {
